...
public class SortBlockedItemsTest {
	...
	public void blockedItemsAreSortedByPriority() throws Exception {
		...
		TestRunListener.init(new ExpectedItem(downstreamProject.getName(), 1), new ExpectedItem(upstreamProject.getName(), 2), new ExpectedItem(downstreamProject.getName(), 1));
		
		j.jenkins.rebuildDependencyGraph();
		
		// Locking makes sure that the queue sees both builds at the same time even though upstreamProject entered the queue first
		Queue.withLock(() -> {
